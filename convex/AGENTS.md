...
- `tickets.ts` owns reservation ticket queries, public Terms/payment gates, confirmed proof-receipt consumption, paginated/filterable dashboard queries, export queries, mutations, persisted status normalization, searchable ticket index fields, index-field backfill, and legacy ticket import.
...
- `conversations.ts` owns durable web/SMS conversation state, transcripts, agent-run audit records, rate limits, staff inbox queries, and human controls.
