...
function buildEmail({ ticket, settings, ticketUrl }) {
  ...
  const hasRetailScreenshot = Boolean(ticket.retailPriceScreenshotKey);
  const htmlRows = rows
    .map(([label, value]) => `
      <tr>
  ...
  const text = [
    `Welcome ${primaryGuest},`,
    "",
    "Your Ritz-Carlton private rate is ready.",
    "",
    ...rows.map(([label, value]) => `${label}: ${value}`),
    ...(hasRetailScreenshot ? ["", "Retail price screenshot: attached"] : []),
    "",
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#fffaf3;border:1px solid #eadfce;border-radius:24px;overflow:hidden;">
              <tr>
                <td style="background:#211b17;color:#ffffff;padding:30px;">
                  ...
                  <h1 style="margin:0;font-size:30px;line-height:1.12;font-weight:700;">Welcome, ${escapeHtml(primaryGuest)}</h1>
              ...
              <tr>
                <td style="padding:14px 30px 24px;">
                  ...
                  ${hasRetailScreenshot ? `<p style="margin:14px 0 0;color:#7a7067;font-size:12px;line-height:1.6;">The Ritz website retail price screenshot is attached to this email and available on your guest ticket.</p>` : ""}
                </td>
              </tr>
              <tr>
...
async function sendPriceSentStaffCopy({ client, ticket, settings, email, attachments, subject }) {
  if (!settings.emailAlertsEnabled) {
    return { sent: false, skipped: true, reason: "Email alerts are disabled" };
  }
  if (!settings.priceSentStaffAlertEnabled) {
    return { sent: false, skipped: true, reason: "Price sent staff copy is disabled" };
  }
  if (ticket.priceSentStaffEmailSentAt) {
    return { sent: false, skipped: true, reason: "Price sent staff copy already sent" };
  }
  ...
  const priceSentStaffEmailSentAt = new Date().toISOString();
  const updated = await client.mutation(api.tickets.update, {
    id: ticket.id,
    data: { priceSentStaffEmailSentAt },
...
export async function sendPriceSentEmail({ client, ticket, origin }) {
  ...
  if (!ticket.email) {
    return { sent: false, skipped: true, reason: "Ticket has no guest email" };
  }
  ...
  const attachments = [];
  const subject = "Your Ritz-Carlton private rate is ready";

  if (ticket.retailPriceScreenshotKey) {
    const extension = getExtensionFromR2Key(ticket.retailPriceScreenshotKey);
    attachments.push(await getR2ObjectAttachment(
      ticket.retailPriceScreenshotKey,
      `ritz-retail-price-screenshot.${extension}`
    ));
  }
  ...
  if (!settings.priceSentGuestEmailEnabled) {
    result = { sent: false, skipped: true, reason: "Guest price sent email is disabled" };
  } else if (ticket.priceSentEmailSentAt) {
    result = { sent: false, skipped: true, reason: "Price sent email already sent" };
  } else {
    await sendResendEmail({
      to: ticket.email,
      subject,
    ...
    currentTicket = await client.mutation(api.tickets.update, {
      id: ticket.id,
  ...
  let staffEmail;
  try {
    staffEmail = await sendPriceSentStaffCopy({
      client,
    ...
    if (staffEmail.ticket) currentTicket = staffEmail.ticket;
  } catch (error) {
    staffEmail = {
      sent: false,
      error: error.message || "Failed to send price sent staff copy",
    };
  }

  return { ...result, ticket: currentTicket, staffEmail };
}
