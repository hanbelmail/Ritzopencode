...
import { buildSmsInitialDisclosure, withDefaultSettings } from "@/lib/defaults";
import { getConvexServiceKey } from "@/lib/convex-server";
import { honoluluToday, resolveAvailabilityMonth, resolveStayDates } from "@/lib/sara-date-resolver";
import { buildSaraPaymentInstructionsReply } from "@/lib/sara-payment-instructions";
...
const tools = [
  ...
  },
  {
    ...
    description: "Check whether the one private residence is currently available for future Hawaii stay dates. The server preserves or infers the guest's year deterministically. This does not hold dates.",
    strict: true,
    parameters: objectSchema({
  ...
  {
    type: "function",
    name: "list_month_availability",
    description: "List the contiguous available check-in/check-out ranges for one requested calendar month in Hawaii. Use for broad month availability questions; the server preserves or infers the guest's year deterministically. This does not hold dates.",
    strict: true,
    parameters: objectSchema({
...
export async function sendAcceptedPaymentInstructions({ client, publicId, accessTokenHash, inboundMessageId, origin }) {
  const serviceKey = getConvexServiceKey();
  const context = await client.query(api.conversations.getContext, { serviceKey, publicId, accessTokenHash });
  ...
  const proofUploadUrl = new URL(`/ticket/${payment.ticketId}`, origin).toString();
  const reply = buildSaraPaymentInstructionsReply({ methods: payment.methods, proofUploadUrl });
  ...
  const message = await client.mutation(api.conversations.appendOutbound, {
    ...
    metadata: {
      deterministic: true,
      action: "payment_instructions_after_terms_acceptance",
      termsVersion: payment.termsVersion,
      termsHash: payment.termsHash,
    },
    expectedControlVersion: context.conversation.controlVersion || 0,
  });

  return { skipped: false, reply, message, outboundMessageId, handedOff: false };
}

async function executeTool({ client, serviceKey, publicId, inboundMessageId, origin, settings, controlVersion, messages, currentHawaiiDate, name, args }) {
  switch (name) {
    ...
    case "check_availability": {
      const stay = resolveStayDates({ checkIn: args.check_in, checkOut: args.check_out, messages, today: currentHawaiiDate });
    ...
    case "list_month_availability": {
      const resolved = resolveAvailabilityMonth({ month: args.month, messages, today: currentHawaiiDate });
      return client.query(api.sara.listMonthAvailability, {
        serviceKey,
    ...
    case "get_booking_terms": {
      ...
      return {
        ...
        requiredAgreement: terms.requiredAgreement,
        smsAcceptanceInstruction: terms.smsAcceptanceInstruction,
        smsAcceptanceContract: terms.smsAcceptanceContract,
      };
    }
    ...
    default:
      throw new Error(`Unsupported Sona tool: ${name}`);
  }
}
...
export async function runSaraAgent({ client, publicId, accessTokenHash, inboundMessageId, origin }) {
  ...
  if (!context) throw new Error("Conversation not found");
  if (!context.conversation.aiEnabled || ["human_required", "closed"].includes(context.conversation.status)) {
    return { skipped: true, reason: "Sona is paused for this conversation" };
  }

  if (context.conversation.termsPresentedAt) {
    const termsReply = await client.mutation(api.sara.processTermsReply, {
      serviceKey,
      publicId,
      messageId: inboundMessageId,
      expectedControlVersion: context.conversation.controlVersion || 0,
    });
    if (["retry_terms_acceptance", "web_control_required", "terms_presentation_unconfirmed"].includes(termsReply.status)) {
      const reply = termsReply.status === "terms_presentation_unconfirmed"
        ? `I couldn't verify delivery of the Terms message. Please ask me to send the Terms again.`
        : termsReply.status === "web_control_required"
        ? `Please use the Terms checkbox and Accept Terms button shown with the published Terms link.`
        : termsReply.matchPolicy === "explicit_allowlist"
          ? `I couldn't record that as acceptance. ${termsReply.requiredAgreement}`
          : termsReply.matchPolicy === "case_whitespace"
          ? `I couldn't record that as acceptance. Please reply with this complete phrase (capitalization does not matter):\n${termsReply.requiredAgreement}`
          : `I couldn't record that because the legacy wording must match exactly. Please reply with this exact line:\n${termsReply.requiredAgreement}`;
      const outboundMessageId = `sara:${inboundMessageId}`;
      const message = await client.mutation(api.conversations.appendOutbound, {
        serviceKey,
        publicId,
        ...
        metadata: {
          deterministic: true,
          action: termsReply.status === "web_control_required"
            ? "terms_web_control_required"
            : termsReply.status === "terms_presentation_unconfirmed"
              ? "terms_presentation_unconfirmed"
              : "terms_phrase_retry",
        },
        expectedControlVersion: context.conversation.controlVersion || 0,
      });
    ...
    if (termsReply.status === "accepted") {
      return sendAcceptedPaymentInstructions({ client, publicId, accessTokenHash, inboundMessageId, origin });
    }
  }

  const savedSettings = await client.query(api.settings.get, { serviceKey });
  const settings = withDefaultSettings(savedSettings);
  const model = settings.saraModel || process.env.OPENAI_MODEL || DEFAULT_MODEL;
  const currentHawaiiDate = honoluluToday();
  ...
  try {
    let response = await openai.responses.create({
      model,
      instructions: buildSaraInstructions({ settings, ...context, currentHawaiiDate }),
      input,
      tools,
    ...
    while (true) {
      ...
      if (response.status === "incomplete" || response.incomplete_details) throw new Error("Sona returned an incomplete response");
      const calls = response.output.filter((item) => item.type === "function_call");
      if (!calls.length) {
        let reply = formatReply(response.output_text, context.conversation.channel);
        if (!reply) throw new Error("Sona returned an empty response");

        const termsPresented = !authorizedPayment && toolLog.some((entry) => entry.name === "get_booking_terms" && entry.ok);
        let presentedTerms = null;
        if (authorizedPayment) {
          reply = buildSaraPaymentInstructionsReply(authorizedPayment);
        } else if (termsPresented) {
          presentedTerms = await client.query(api.sara.getBookingTerms, { serviceKey, publicId });
          const termsUrl = new URL(`/terms/${encodeURIComponent(presentedTerms.version)}`, origin);
          const termsMessage = context.conversation.channel === "sms"
            ? `Review the published booking Terms version ${presentedTerms.version}: ${termsUrl}\n\n${presentedTerms.smsAcceptanceInstruction}`
            : `Please review the published booking Terms: ${termsUrl}\n\nUse the Terms checkbox and Accept Terms button shown below when you are ready.`;
          reply = formatReply(termsMessage, context.conversation.channel);
        }

        const includesInitialDisclosure = Boolean(run.smsInitialDisclosureClaimed);
        if (includesInitialDisclosure) {
          const disclosedReply = `${buildSmsInitialDisclosure()}\n\n${reply}`;
          reply = authorizedPayment ? disclosedReply : formatReply(disclosedReply, context.conversation.channel);
        }
        ...
        const message = await client.mutation(api.conversations.appendOutbound, {
          ...
          metadata: {
            model,
            promptVersion: SARA_PROMPT_VERSION,
            ...(includesInitialDisclosure ? { initialDisclosure: true } : {}),
            ...(authorizedPayment ? {
              deterministic: true,
              action: "payment_instructions_requested",
              termsVersion: authorizedPayment.termsVersion,
              termsHash: authorizedPayment.termsHash,
            } : {}),
            ...(termsPresented && context.conversation.channel === "sms" ? { termsAcceptanceContract: presentedTerms.smsAcceptanceContract } : {}),
          },
      ...
      if (toolRounds >= MAX_TOOL_ROUNDS) throw new Error("Sona exceeded the tool-call limit");
      toolRounds += 1;
      ...
      for (const call of calls) {
        ...
        try {
          ...
          if (call.name === "get_payment_instructions") authorizedPayment = output;
    ...
    await client.mutation(api.conversations.finishRun, {
      ...
      inputTokens: totalInputTokens,
      outputTokens: totalOutputTokens,
      error: String(error.message || "Sona failed").slice(0, 1000),
    });
    throw error;
  }
}
