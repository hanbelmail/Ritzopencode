...
export async function runSaraAgent({ client, publicId, accessTokenHash, inboundMessageId, origin }) {
  ...
  if (context.conversation.termsPresentedAt) {
    ...
    if (termsReply.status === "retry_exact") {
      const reply = context.conversation.channel === "sms"
        ? `I couldn't record that because the wording must match exactly. Please reply with this exact line:\n${termsReply.requiredAgreement}`
        : `I couldn't record that wording. Please use the Terms checkbox and Accept Terms button shown with the published Terms link.`;
      ...
      const message = await client.mutation(api.conversations.appendOutbound, {
        ...
        metadata: { deterministic: true, action: "terms_exact_phrase_retry" },
        expectedControlVersion: context.conversation.controlVersion || 0,
      });
      ...
      if (!calls.length) {
        ...
        if (termsPresented) {
          ...
          const termsMessage = context.conversation.channel === "sms"
            ? `Review the published booking Terms: ${termsUrl}\n\nTo accept, reply with this exact line:\n${presentedTerms.requiredAgreement}`
