...
export function assertPdfContentType(contentType, label = "File") {
...
export function createBookingConfirmedHotelAlertAttachmentKey(fileName = "booking-confirmed-attachment.pdf") {
  return `booking-confirmed-hotel-alerts/${crypto.randomUUID()}.${extensionFromFileName(fileName, "pdf")}`;
}
...
export async function createBookingConfirmedHotelAlertAttachmentUploadUrl({ key, contentType }) {
  const { bucketName } = getR2Env();
  const command = new PutObjectCommand({
    Bucket: bucketName,
    Key: key,
...
export async function inspectPaymentProofObject(key) {
  const { bucketName } = getR2Env();
  const response = await getR2Client().send(new HeadObjectCommand({ Bucket: bucketName, Key: key }));
  const size = Number(response.ContentLength || 0);
  const contentType = String(response.ContentType || "");
  if (!size || size > 10 * 1024 * 1024) throw new Error("Payment proof must be a non-empty image no larger than 10 MB");
  assertImageContentType(contentType, "Payment proof");
  return { size, contentType, etag: String(response.ETag || "").replaceAll('"', "") || undefined };
}
...
export function isBookingConfirmedHotelAlertAttachmentKey(key) {
  return typeof key === "string" && key.startsWith("booking-confirmed-hotel-alerts/");
}
